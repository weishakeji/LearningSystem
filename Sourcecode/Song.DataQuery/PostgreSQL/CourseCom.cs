...
namespace Song.DataQuery.PostgreSQL
{
    public class CourseCom
    {
        ...
        public List<Course> CourseForStudent(int stid, string sear, int state, bool? enable, bool? istry, int size, int index, out int countSum)
        {
            ...
            if (enable != null) sql1 = sql1.Replace("{{enable}}", @"sc.""Stc_IsEnable"" = " + ((bool)enable).ToString().ToLower());
            else
            ...
            //试用
            sql1 = sql1.Replace("{{istry}}", istry != null ? @"sc.""Stc_IsTry"" = " + ((bool)istry).ToString().ToLower() : "1=1");
            ...
            if (!string.IsNullOrWhiteSpace(sear)) sql_total += @" where ""Cou_Name"" ILIKE '%" + sear + "%'";
        ...
        public List<Course> RankIncome(int orgid, long sbjid, DateTime? start, DateTime? end, int size, int index, out int countSum)
        {
            ...
            select COALESCE(b.count,0) as count, c.* from ""Course"" as c left join 
            ...
            sqljquery = @"  select * from
              ...
              (SELECT ""Ac_ID"", MAX(""Cou_ID"") as Cou_ID, MAX(""Lss_LastTime"") as lastTime, 
                     ...
                     (CASE WHEN MAX(""Lss_Duration"")> 0 THEN
                          (1000 * sum(""Lss_StudyTime""::float) / sum(""Lss_Duration""::float)) * 100
                          ELSE 0 END
                      ) AS complete
    ...
    on acc.""Ac_ID"" = lss.""Ac_ID"" and acc.""Cou_ID"" = lss.Cou_ID";
        ...
        {
            //支持postgresql
            string sql = @"select SUM(total) from (select COALESCE(b.total,0) as total from ""Course"" as c left join 
                            (SELECT ""Cou_ID"", sum(""Stc_Money"") as total
                              FROM ""Student_Course"" where  {start} and {end} group by ""Cou_ID"") as b
                              on c.""Cou_ID"" = b.""Cou_ID"" where  ""Org_ID"" = {orgid} and 1 = 1 ) as tm ";
            sql = sql.Replace("{orgid}", orgid.ToString());
        ...
        /// <returns></returns>
        private int CourseOfCount(int orgid, long sbjid, int thid, bool? isuse, bool? isfree)
        {
