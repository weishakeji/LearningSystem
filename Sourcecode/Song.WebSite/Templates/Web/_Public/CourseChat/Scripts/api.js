...
(function () {
    ...
    var methods = {
        ...
        hash: function (val, url) {
            ...
            var hashValue = url.indexOf('#') > -1 ? url.substring(url.lastIndexOf('#') + 1) : '';
        ...
        //判断变量是否是对象
        isobj: function (obj) { return Object.prototype.toString.call(obj) === '[object Object]'; },
        //在线浏览pdf文件
        ...
        //网页是否处于微信内置浏览器
        isWeixin: function () { return window.navigator.userAgent.toLowerCase().match(/MicroMessenger/i) == 'micromessenger'; },
        //网页是否处于微信小程序内置浏览器
        isWeixinApp: function () { return window.navigator.userAgent.toLowerCase().match(/miniProgram/i) == 'miniprogram' },
        //是否是IE浏览器
        ...
        isapp: function () {
            //判断变色龙打包工具
            var isbsl = window.navigator.userAgent.toLowerCase().indexOf('bsl') > -1;
            return isbsl;
        },
        //对象转Json
    ...
    apiObj.prototype.apipages = function () {
        ...
        if (name == "") {
            ...
            if (window.top.$pagebox) {
                ...
                while (box != null) {
                    boxs.push(box);
                    var win = box.document();
                    list.push(win.location.pathname);
                    box = pagebox.get(box.pid);
                }
                if (boxs.length > 0) {
                    var iframe = window.top.$dom('iframe[name=\'' + boxs[boxs.length - 1].pid + '\']');
                    if (iframe.length > 0) {
    ...
    apiObj.prototype.login = {
        ...
        item: function (target) {
            var status = methods.storage(this.storagename);
        ...
        tokens: function (key, code, id, dura) {
            ...
            if (arguments.length >= 2) {
                ...
                var index = status.findIndex(function (el) { el.key == key });
                var item = index < 0 ? { key: key } : status[index];
                var time = new Date();
        ...
        'fresh': function (target, succfunc, errfunc) {
            var th = this;
            var interval = 10;        //每次递交刷新的间隔时间，单位秒
            var items = target == null ? th.item() : [th.item(target)];
            for (var i = 0; i < items.length; i++) {
                var el = items[i];
                if (el == null || JSON.stringify(el) == '{}') continue;
                var dur = el.duration + 1;
        ...
        'initial': function () {
            ...
            window.addEventListener('focus', function () { window.loginstatus_duration_allow = true });
            window.addEventListener('blur', function () { window.loginstatus_duration_allow = false });
            window.setInterval(function () {
                if (window.self !== window.top) return;
                if (window.loginstatus_duration_allow) {
                    var count = window.loginstatus_duration_count;
    ...
    apiObj.prototype.url = {
        ...
        params: function (url) {
            ...
            var paras = url.substring(url.lastIndexOf("?") + 1).split('&');
            for (var q in paras) {
                if (paras[q].indexOf('=') < 0) continue;
                var key = paras[q].substring(0, paras[q].indexOf('='));
                var val = paras[q].substring(paras[q].indexOf('=') + 1);
        ...
        get: function (url, key) {
            ...
            //var res = values.find(function (t) { return t.key.toLowerCase() === key.toLowerCase(); });
            var res = values.filter(function (t) {  
                return t.key.toLowerCase() === key.toLowerCase();  
            })[0] || undefined;  
            return res === undefined ? '' : this.filter(res.val, key);
        },
        set: function (url, key, value) {
            if (key.constructor === Object) {
                for (var k in key) url = this.set(url, k, key[k]);
                return url;
            }
            var values = this.params(url);
            //var res = values.find(function (t) { return t.key.toLowerCase() === key.toLowerCase(); });
        ...
        filter: function (val, key) {
            ...
            return val;
            var rex = /\'|\"|\.\.|>|and|exec|insert|select|delete|update|count|\*|\%|chr|mid|master|truncate|char|declare|script|frame|\;|or|\-|\+|\)|etc|style|expression/gi;
            var result = val.match(rex);
            ...
            return new Promise(function (resolve, reject) {
                ...
                request.onsuccess = function (event) {
                    ...
                    if (!db.objectStoreNames.contains(p.store)) {
                        ...
                        if (p.active == 'clear') {
                            return th.del(way, para).then(function (d) { return reject(d); }).catch(function (err) { return reject(err); });
                        }
