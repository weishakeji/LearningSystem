...
(function () {
    ...
    */
        ...
        css: function (src, callback, tagName) {
            webdom.load.arraySync(function (one, i, c) {
                ...
                webdom("link[type='text/css']").each(function () {
                    var href = webdom(this).attr("href").toLowerCase();
    ...
    webdom.iscache = function () {
        var template = webdom('meta[template]');
        var iscache = template.attr("cache");
        return iscache == 'true';
    };
    //页面路由，和地址栏Url有一定区别
    ...
    webdom.device = function () {
        var template = webdom('meta[device]');
    ...
    webdom.corejs = function (func, jsfile) {
        //要加载的js 
        var arr = ['vue.min', 'polyfill.min', 'axios_min',
            'api',      //axios封装
            'utils',    //工具类
            'position'  //获取GPS定位数据
        ];
