$ready(function () {
    ...
    window.vapp = new Vue({
        ...
        data: {
            ...
            datas: [],           //数据列表
            finished: false,            
            query: { 'acid': '', 'start': '', 'end': '', 'type': '-1', 'search': '', 'size': 10, 'index': 0 },
            total: 0
        },
        mounted: function () {
        ...
        computed: {
            ...
            details: function () {
                ...
                for (var i = 0; i < datas.length; i++) {
                    var str_mh = datas[i]["Ca_CrtTime"].format(fmt);
                    let isexist = false;
        ...
        methods: {
            onload: function () {
                ...
                console.log(query);
                $api.get('Coupon/PagerForAccount', query).then(function (req) {
                    if (req.data.success) {
            ...
            btnDelete: function (item) {
                ...
                this.$dialog.confirm({
                    ...
                    $api.delete('Coupon/DeleteSingle', { 'id': item.Ca_ID }).then(function (req) {
                        if (req.data.success) {
                            var result = req.data.result;
                            if (result == true) {
            ...
            remark: function (item) {
                var remark = $api.trim(item.Ca_Remark) != "" ? item.Ca_Remark : item.Ca_Info;
                return remark;
            }
        }
    });
