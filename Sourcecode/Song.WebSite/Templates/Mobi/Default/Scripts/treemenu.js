...
(function (win) {
	var treemenu = function (param) {
		...
		this._datas = JSON.stringify(this.datas);
		this._restructure();
		//初始化并生成控件
	...
	fn.isnull = function () {
		if (this.datas.length < 1) return true;
			...
			if (this.datas.length == 1 && this.datas[0].type == 'loading') return true;
		return false;
	};
	...
	fn._builder = {
		...
		body: function (obj) {
			...
			for (var i = 0; i < obj.datas.length; i++) {
				...
				//右侧菜单的大标题
				var tit = area.add('tree_tit').html(item.title).attr("treeid", item.id);
				//节点点击事件
				...
				if (item.childs) {
					var treebox = area.add('tree_area_box');
					for (var j = 0; j < item.childs.length; j++) {
						_addchild(treebox, item.childs[j], obj);
					}
				}
			}
			//添加树形的子级节点
	...
	fn._createNode = function (item, box) {
		...
		span.width('calc(100% - ' + ((item.level - 1) * 20 + 10) + 'px)');

		//如果有下级节点
		if (item.childs && item.childs.length > 0) {
			var folder = node.add("folder").addClass('open');
			folder.click(function (event) {
				event.stopPropagation();
				var n = event.target ? event.target : event.srcElement;
				var folder = $dom(n);
				if (folder.hasClass('open'))
					folder.attr('class', 'close');
				else
					folder.attr('class', 'open');
		...
		if (item.type == 'link') return node;
		//节点点击事件
		node.find("span").click(function (event) {
			event.stopPropagation();
			var n = event.target ? event.target : event.srcElement;
	...
	fn._calcLevel = function (items, level) {
		for (var i = 0; i < items.length; i++) {
			...
			if (!item.ico || item.ico == '') item.ico = 'e765';
