    ...
    window.vapp = new Vue({
        ...
        methods: {
            ...
            getdatainfo: function () {
                var th = this;
                $api.get('Learningcard/SetDataInfo', { 'id': th.form.lsid }).then(function (req) {
                    if (req.data.success) {
            ...
            goback: function (row) {
                ...
                if (row) {
                    ...
                    if (th.selects.length < 1) {
                        ...
                        var msg = "";
                        if (th.selects.length > num) {
                            msg = "您选中了" + th.selects.length + "个学习卡，其中有" + (th.selects.length - num) + "个未使用或已经回滚。"
                            msg += "<br/>在可以回滚的" + num + "个学习卡中，请选择回滚方式。"
                        } else {
                            msg = "您选中了" + th.selects.length + "个学习卡，请选择回滚方式。"
                        }
                        th.$confirm(msg, '确认', {
                            distinguishCancelAndClose: true,
                            ...
                            for (var i = 0; i < th.selects.length; i++) {
                                if (th.selects[i].Lc_IsUsed)
                                    th.gobackFunc(th.selects[i], true);
                            }
                        }).catch(action => {
                            if (action == 'cancel') {
                                for (var i = 0; i < th.selects.length; i++) {
                                    if (th.selects[i].Lc_IsUsed)
                                        th.gobackFunc(th.selects[i], false);
                                }
                            }
                        });
                    }
