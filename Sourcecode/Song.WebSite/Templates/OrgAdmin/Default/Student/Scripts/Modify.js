        ...
        window.vapp = new Vue({
            ...
            created: function () {
                ...
                th.getaccount().then(function (acc) {
                    th.account = acc;
                    if ($api.isnull(th.account.Ac_Birthday) || $api.getType(th.account.Ac_Birthday) != 'Date')
                        th.account.Ac_Birthday = '';
                    //获取机构信息
                    th.getorgan();
            ...
            methods: {
                ...
                birthdaychange: function (val) {
                    ...
                    // 判断今年生日是否已过
                    const curBirth = new Date(today.getFullYear(), birthDate.getMonth(), birthDate.getDate());
                    if (curBirth > today) {
                        age -= 1;
                    }
                    this.account.Ac_Age = age < 0 ? 0 : age;
                },
                //当年龄改变时，出生年月也变化
                agechange: function (val) {
                    if (val < 0) this.account.Ac_Age = 0;
                    if (val > 200) this.account.Ac_Age = 200;
                    const today = new Date();
                    const curYear = today.getFullYear();
                    const curMonth = today.getMonth();
                    const curDay = today.getDate();
                    // 预估出生年份
                    let birthYear = curYear - val;
                    let birthDate = new Date(birthYear, curMonth, curDay);
                    this.account.Ac_Birthday = birthDate;
                },
                //绑定手机号
