...
namespace Song.Site.Pay
{
    ...
    {
        //钱数
        private Decimal money = (Decimal)(WeiSha.Core.Request.Form["money"].Double ?? 0);
        //验证码
        string vpaycode = WeiSha.Core.Request.Form["vpaycode"].String; 
        //支付接口的id
        private int paiid = WeiSha.Core.Request.Form["paiid"].Int32 ?? 0;
        //学员id
        private int acid = WeiSha.Core.Request.Form["acid"].Int32 ?? 0;
        //是否校验验证码
        private bool isVerify = WeiSha.Core.Request.QueryString["isVerify"].Boolean ?? true;
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            //{
            //    context.Response.Redirect(WeiSha.Core.Request.Page.AddPara(from, "err=1", "money=" + money, "paiid=" + paiid));
            //    return;
            //}
            ////验证码不正确
            ...
            {
                //设置项不存在
                context.Response.Redirect(WeiSha.Core.Request.Page.AddPara(from, "err=2", "money=" + money, "paiid=" + paiid));
                return;
            }
            else
            {
        ...
        private void Alipaywap(Song.Entities.PayInterface pi, Song.Entities.MoneyAccount ma)
        {
            ////////////////////////////////////////////请求参数////////////////////////////////////////////
            //回调域
            string domain = "http://" + WeiSha.Core.Server.Domain + ":" + WeiSha.Core.Server.Port + "/";
            ...
            //订单名称
            Song.Entities.Accounts account = Business.Do<IAccounts>().AccountsSingle(this.acid);
            string name = string.IsNullOrWhiteSpace(account.Ac_Name) ? "" : account.Ac_Name;
            string subject = name + "(" + account.Ac_AccName + ")充值";
            //必填
            //付款金额
        ...
        {
            //取图片验证码
            string imgCode = WeiSha.Core.Request.Cookies["vpaycode"].ParaValue;
            //取员工输入的验证码
            string userCode = new WeiSha.Core.Param.Method.ConvertToAnyValue(vpaycode).MD5;
            //验证
