...
function ajaxLoaddata(){
	...
	$.post(window.location.href, { size: size, index: index },function(requestdata){		
        ...
        for (var i = 0; i < data.items.length; i++) {
			...
			li.setAttribute("acid",d.Ac_Id);
			//相关数据
			var defimg=document.getElementById("def-img");
			var img=d.Ac_Photo!="" ? d.Ac_Photo : defimg.getAttribute("src");			
			//
			var html='<a href="javascript:;">';
			html+='<img class="mui-media-object mui-pull-left" src="'+img+'"/>';
			html+='<div class="mui-media-body mui-ellipsis">'+unescape(d.Ac_Name);
			html+='<p class="mui-ellipsis">'+unescape(d.Ac_AccName)+'</p>';
			html+='</div>';
			html+='</a>';
            li.innerHTML = html;
		 ...
		 mui('body').on('tap', '.news-item', function () {
				var id=$(this).attr("acid");				
				//new PageBox("学员详情", "article.ashx?id="+id, 100, 100, "url").Open();
		});
	});	
}