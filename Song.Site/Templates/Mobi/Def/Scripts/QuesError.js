...
function ajaxLoaddata() {
    ...
    $.post("QuesError.ashx", { size: size, index: index, action: "list" }, function (requestdata) {
        ...
        for (var i = 0; i < data.items.length; i++) {
            ...
            //相关数据           
            var date = new Date(d.Tr_CrtTime).Format("MM-dd"); //资金流水的创建日期
            var time = new Date(d.Tr_CrtTime).Format("hh:mm"); //资金流水的创建时时间			
            //
            var html = '';
        ...
        mui('body').on('tap', '.mui-btn', function (event) {
            ...
            if (action == "del") {
                mui.confirm('确认清除该试题记录？', '清除', ['取消', '确认'], function (e) {
                    if (e.index == 1) {
                        $.post("QuesError.ashx", { action: "delete", id: qid }, function (data) {
                            if (data == "1") {
                                mui.toast('清除成功！', { duration: 1000, type: 'div' });
                            } else {
            ...
            if (action == "edit") {
                 new PageBox("试题回顾练习", "QuesErrorReview.ashx?qid=" + qid, 100, 100, "url").Open();
            }
            //
        });
    });
