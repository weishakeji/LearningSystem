...
function setBtnEvent() {
    ...
    $("#formRegist").submit(function () {
        ...
        if (!isSms) {
            var url = window.location.href;
            ajax.post(url, { action: "register1", openid: openid, mobi: mobi }, function (requestdata) {
                var data = eval("(" + requestdata + ")");
...
function closeWinBox() {
    ...
    //var msg = new window.top.PageBox();
    //msg.Close();
    //来源页
    var from = $("input[name=from]", window.top.document);
    var url = $.trim(from.attr("path"));
    if (url.length < 1) url = window.top.location.href;
    url = url.replace("#", "");
    url = $().setPara(url, "tick", new Date().getTime());
