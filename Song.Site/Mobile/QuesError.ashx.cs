...
namespace Song.Site.Mobile
{
    ...
    public class QuesError : BasePage
    {
        ...
        private void list()
        {
            ...
            int sumcount = 0;
            Song.Entities.Questions[] ques = Business.Do<IStudent>().QuesPager(st.Ac_ID, 0, currCourse.Cou_ID, -1, -1, size, index, out sumcount);
            ...
            for (int n = 0; n < ques.Length; n++)
            {
                ...
                if (!string.IsNullOrWhiteSpace(ques[n].Qus_Title) && ques[n].Qus_Title.Length >= 50)
                {
                    ques[n].Qus_Title = ques[n].Qus_Title.Substring(0, 50) + "...";
                    ques[n].Qus_Title = trim(ques[n].Qus_Title);
                }
                ques[n].Qus_Items = ques[n].Qus_Explain = ques[n].Qus_ErrorInfo = ques[n].Qus_WrongInfo = "";
                json += ques[n].ToJson() + ",";
            }
